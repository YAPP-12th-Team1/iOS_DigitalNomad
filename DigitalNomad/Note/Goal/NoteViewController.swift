...
class GoalViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //데이터 초기화
        self.goalLists = (realm.objects(ProjectInfo.self).last?.goalLists)
        ...
        //GoalList 초기화, 컨테이너뷰 안에 추가
        self.addGoalTableView()
    }
    ...
    //MARK:- 사용자 정의 메소드
    func addGoalTableView() {
        guard let goalTableView = GoalTableView.instanceFromXib() as? GoalTableView else { return }
        goalTableView.tableView.delegate = self
        goalTableView.tableView.dataSource = self
        goalTableView.tableView.emptyDataSetSource = self
        goalTableView.tableView.emptyDataSetDelegate = self
        self.containerView.addSubview(goalTableView)
        goalTableView.snp.makeConstraints { (make) in
            make.top.bottom.left.right.equalToSuperview()
        }
    }
    
    func addWishCollectionView() {
...
extension GoalViewController: GoalCellDelegate {
    func clickCheckBox(_ sender: BEMCheckBox) {
        let id = sender.tag
        let query = NSPredicate(format: "id = %d", id)
        guard let result = self.goalLists?.filter(query).first else { return }
